    ...
    def draw(self):
        ...
        for y, column in enumerate(self.board):
            for x, item in enumerate(column):
                self.spots[x + y * 3].text = (
                    "o" if item == 2 else ("x" if item == 1 else "")
                )
        ...
        for i in range(3):
            pygame.draw.line(
                self.WIN,
                pgh.BLACK,
                (0, i * (self.H // 3) - (line_width // 2)),
                (self.W, i * (self.H // 3) - (line_width // 2)),
                line_width,
            )
            pygame.draw.line(
                self.WIN,
                pgh.BLACK,
                (i * (self.W // 3) - (line_width // 2), 0),
                (i * (self.W // 3) - (line_width // 2), self.H),
                line_width,
            )
            ...
            if event.type == pygame.QUIT:
                ...
                for button in self.spots:
                    if button.rect.collidepoint(event.pos) and not button.pressed:
                        ...
                        self.board[event.pos[0] // (self.W // 3)][
                            event.pos[1] // (self.H // 3)
                        ] = (self.turn + 1)
                        self.turn += 1
                        pygame.display.set_caption(
                            f"""Tic Tac Toe | {"o's" if self.turn == 1 else "x's"} turn"""
                        )
                        self.turn %= 2
