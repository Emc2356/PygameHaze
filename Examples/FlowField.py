...
class FlowField:
    def __init__(self):
        ...
        self.perlin_vectors: np.ndarray = np.ndarray((self.columns * self.rows, 2), dtype=np.float64)
    ...
    def draw(self) -> None:
        ...
        for p, v, pp in self.particles:
            i, j = int(p.y // self.w), int(p.x // self.w)  # find the particle's location in the flowfield
